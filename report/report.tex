...
\subsection*{Dataset}
For the purpose of this project we first curated a dataset @gasper kolbezen, če tukej kej več poveš

\subsection*{Vector storage and retrieval}
For vector storage and retrieval, we utilize \textbf{TimescaleDB} with the \textbf{pgvector} extension, managed through the \texttt{timescale-vector} Python client.
...
To ensure efficient and scalable retrieval, we implemented a \textbf{Hierarchical Navigable Small World (HNSW)} index using \textbf{cosine similarity} (\texttt{vector\_cosine\_ops}).
