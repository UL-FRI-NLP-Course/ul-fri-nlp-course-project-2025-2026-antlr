...
Document chunks are represented as 1024-dimensional vectors generated by the \textbf{BAAI/bge-m3}\cite{baai3} model, which was selected for its strong performance in multilingual tasks.
...
During development, we tested several embedding models. They achieved similar performance on our harder testing dataset which we developed.
We evaluated \textbf{intfloat/multilingual-e5-base}\cite{mult-e5-base}, which scored 63\%, beaten both by \textbf{bge-m3}\cite{baai3} and Google's \textbf{gemma-embedding-300m}\cite{embedding_gemma_2025}, which both scored near 75\%.
Due to similar performance of the later two, but different licensing conditions, we chose to go with the \cite{baai3}.
Google's model is restricted behind authentication, whereas the \cite{baai3} is available without being authenticated to the HuggingFace,
which makes the inference and community testing simpler.
On the other hand, \cite{embedding_gemma_2025} uses lower dimensionality for embedding vectors, 768, smaller than \cite{baai3} 1024, however, we did not observe
any drastical improvement in speed.
